...
const PaymentVoucherList = () => {
  ...
  const fetchVouchers = useCallback(async () => {
    ...
    return list.filter((voucher) => voucher.type === 'PAYMENT');
  }, []);
  ...
  const filteredVouchers = useMemo(() => {
    return vouchers.filter((voucher) => {
      ...
      if (filters.payeeId) {
        const matchesPayee = voucher.lines.some((line) => (line.debit ?? 0) > 0 && line.ledgerId === filters.payeeId);
        if (!matchesPayee) {
          return false;
        }
      }
  ...
  const primaryPayeeName = (voucher: Voucher) => {
    const payeeLine = voucher.lines.find((line) => (line.debit ?? 0) > 0);
    return payeeLine ? ledgerNameMap.get(payeeLine.ledgerId) ?? payeeLine.ledgerId : '—';
  };

  const handleDelete = async (voucher: Voucher) => {
    if (!window.confirm(`Delete payment voucher "${voucher.number}"?`)) return;
    try {
    ...
    <Stack spacing={2}>
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', md: 'center' }} spacing={1.5}>
        ...
        <Stack direction="row" spacing={1}>
          ...
          {canCreate && (
            <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate('/vouchers/money/new?type=PAYMENT')}>
              New Payment Voucher
            </Button>
          )}
        </Stack>
          ...
          <Stack spacing={2}>
            <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
              ...
              >
                ...
                {ledgers
                  .filter((ledger) => !ledger.isCashBank)
                  .map((ledger) => (
                  ...
                  filteredVouchers
                    ...
                    .map((voucher) => (
                      <TableRow key={voucher.id} hover>
                        ...
                        <TableCell>{primaryPayeeName(voucher)}</TableCell>
                        ...
                        <TableCell align="right">
                          {canCreate && (
                            <Tooltip title="Edit">
                              <IconButton
                                onClick={() =>
                                  navigate(`/vouchers/payment-vouchers/${voucher.id}/edit`, {
                                    state: { voucherId: voucher.id, voucherType: 'PAYMENT' },
                                  })
                                }
                              >
