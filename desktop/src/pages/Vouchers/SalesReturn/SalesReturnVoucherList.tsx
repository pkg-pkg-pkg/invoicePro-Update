...
const SalesReturnVoucherList = () => {
  ...
  const fetchVouchers = useCallback(async () => {
    ...
    return list.filter((voucher) => voucher.type === 'SALES_RETURN');
  }, []);
  ...
  const filteredVouchers = useMemo(() => {
    return vouchers.filter((voucher) => {
      ...
      if (filters.customerId) {
        const customerLine = voucher.lines.find((line) => (line.credit ?? 0) > 0);
    ...
    <Stack spacing={2}>
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', md: 'center' }} spacing={1.5}>
        ...
        <Stack direction="row" spacing={1}>
          ...
          {canCreate && (
            <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate('/vouchers/sales-return/new')}>
              New Credit Note
            </Button>
          )}
        </Stack>
      ...
      <Dialog open={Boolean(selectedVoucher)} onClose={() => setSelectedVoucher(null)} maxWidth="md" fullWidth>
        ...
        <DialogContent dividers>
          {selectedVoucher ? (
            <Stack spacing={2}>
              <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
                <TextField label="Credit Note Number" value={selectedVoucher.number} fullWidth InputProps={{ readOnly: true }} />
                <TextField
                  label="Date"
...
export default SalesReturnVoucherList;
