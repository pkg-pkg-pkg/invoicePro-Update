...
const ReceiptVoucherForm = () => {
  ...
  const canSubmit =
    ...
    formState.date &&
    formState.number &&
    formState.payerName.trim().length > 0 &&
  ...
  const handlePartySearch = async () => {
    ...
    try {
      // Create/find customer ledger
      const ledgerId = await autoLedgerService.ensureCustomerLedger(formState.payerName.trim());
      
      if (!ledgerId) {
        setError('Customer not found. Please check name and try again.');
        return;
      }

      // Fetch outstanding invoices
      const invoices = await fetchOutstandingInvoices(ledgerId, 'CUSTOMER');
      ...
      setFormState(prev => ({
        ...prev,
        narration: `Receipt against ${invoiceNumbers}`,
      }));
  ...
  const postingPreview = useMemo(() => {
    if (!formState.payerName || totals.selectedInvoices.length === 0) return [];

    const paymentData: PaymentData = {
      partyLedgerId: formState.payerName,
      partyName: formState.payerName,
      partyType: 'CUSTOMER',
      paymentType: 'RECEIPT',
      date: formState.date,
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" spacing={2} alignItems={{ xs: 'flex-start', md: 'center' }}>
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                Process customer receipts against outstanding invoices
              </Typography>
            </Box>
            ...
            <Grid item xs={12} md={4}>
              <Stack direction="row" spacing={1}>
                <TextField
                  ...
                  onChange={(e) => setFormState((prev) => ({ ...prev, payerName: e.target.value }))}
                  fullWidth
                      ...
                      <TableBody>
                        {postingPreview.map((entry, idx) => (
                          <TableRow key={`${entry.ledgerId}-${idx}`}>
                            <TableCell>
                              {entry.ledgerId === 'cash-ledger' ? 'Cash/Bank' : formState.payerName}
                            </TableCell>
