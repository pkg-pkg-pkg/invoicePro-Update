...
const ReceiptVoucherList = () => {
  ...
  const fetchVouchers = useCallback(async () => {
    ...
    return list.filter((voucher) => voucher.type === 'RECEIPT');
  }, []);
  ...
  const filteredVouchers = useMemo(() => {
    return vouchers.filter((voucher) => {
      ...
      if (filters.payerId) {
        const matchesPayer = voucher.lines.some((line) => (line.credit ?? 0) > 0 && line.ledgerId === filters.payerId);
        if (!matchesPayer) {
          return false;
        }
      }
  ...
  const payerName = (voucher: Voucher) => {
    const payerLine = voucher.lines.find((line) => (line.credit ?? 0) > 0);
