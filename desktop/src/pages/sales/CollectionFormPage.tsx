...
export default function CollectionFormPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const narration = [form.narration.trim(), modeToken, refToken, ...invoiceAllocTokens].filter(Boolean).join(' ');
      ...
      if (isEdit && editId) await voucherService.update(editId, payload);
    ...
    <Box sx={{ p: 2 }}>
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} md={6}>
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField {...params} label="Customer *" size="small" placeholder="Search customer…" />
              )}
              noOptionsText={customers.length === 0 ? 'No customers found — add in Customer Master' : 'No match'}
            />
          </Grid>
      ...
      <Paper sx={{ p: 2, mb: 2 }}>
        ...
        {loadingInvoices ? (
          ...
          <Typography variant="body2" color="text.secondary">Select a customer to view unpaid invoices.</Typography>
        ) : invoices.length === 0 ? (
          <Typography variant="body2" color="text.secondary">No outstanding invoices for this customer.</Typography>
        ) : (
          <Table size="small">
            <TableHead>
              <TableRow>
